...
namespace osapi
{
    ...
    void Thread::start()
    {
        ...
        if(getuid() == 0) // Check to see if we are root
        {
            ...
            // Create thread with explicit (non-inherited) scheduling - setting priority will not work otherwise!
            if(pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED) != 0) throw ThreadError();

            // Set priority
            sched_p.sched_priority = static_cast<int>(priority_);
